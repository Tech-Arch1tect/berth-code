  ...
  constructor(
    ...
    if (data.type === "server") {
      ...
      if (permissions && permissions.match(/^[d-][rwx-]{9}$/)) {
        const numericMode =
          BerthTreeItemImpl.convertSymbolicToNumeric(permissions);
      ...
      if (
        data.fileEntry.ownerId !== undefined ||
        data.fileEntry.groupId !== undefined ||
        data.fileEntry.owner ||
        data.fileEntry.group
      ) {
        let ownerInfo = "";
        let groupInfo = "";
      ...
      this.description = data.fileEntry.isDirectory
        ? ""
        : data.fileEntry.displaySize;
...
export class BerthTreeDataProvider implements vscode.TreeDataProvider<BerthTreeItemImpl> {
  ...
  public async createFile(parentItem?: BerthTreeItemImpl): Promise<void> {
    ...
    if (parentItem?.data) {
      if (
        parentItem.data.type === "file" &&
        parentItem.data.fileEntry.isDirectory
      ) {
    ...
    if (!targetServer || !targetStack) {
      vscode.window.showErrorMessage(
        "Please select a stack or folder to create the file in",
      );
      return;
    }
  ...
  public async createFolder(parentItem?: BerthTreeItemImpl): Promise<void> {
    ...
    if (!targetServer || !targetStack) {
      vscode.window.showErrorMessage(
        "Please select a stack or folder to create the folder in",
      );
      return;
    }
  ...
  public async deleteFile(item: BerthTreeItemImpl): Promise<void> {
    ...
    try {
      ...
      this.refresh();
      vscode.window.showInformationMessage(
        `"${fileEntry.name}" deleted successfully`,
      );
    } catch (error) {
      vscode.window.showErrorMessage(
  ...
  public async uploadFile(parentItem?: BerthTreeItemImpl): Promise<void> {
    ...
    if (!targetServer || !targetStack) {
      vscode.window.showErrorMessage(
        "Please select a stack or folder to upload files to",
      );
      return;
    }
  ...
  public async downloadFile(item: BerthTreeItemImpl): Promise<void> {
    if (
      !item?.data ||
      item.data.type !== "file" ||
      item.data.fileEntry.isDirectory
    ) {
  ...
  public async chmodFile(item: BerthTreeItemImpl): Promise<void> {
    if (!item?.data || item.data.type !== "file") {
      vscode.window.showErrorMessage(
        "Please select a file or folder to change permissions",
      );
      return;
    }
    ...
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to change permissions: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  public async chownFile(item: BerthTreeItemImpl): Promise<void> {
    if (!item?.data || item.data.type !== "file") {
      vscode.window.showErrorMessage(
        "Please select a file or folder to change ownership",
      );
      return;
    }
    ...
    const currentOwner =
      fileEntry.owner || `uid:${fileEntry.ownerId || "unknown"}`;
    const currentGroup =
      fileEntry.group || `gid:${fileEntry.groupId || "unknown"}`;
    ...
    if (!ownerInput.trim() && !groupInput.trim()) {
      vscode.window.showInformationMessage(
        "At least one of owner or group must be specified",
      );
      return;
    }
    ...
    try {
      ...
      if (ownerId !== undefined) {
        changes.push(`owner: ${ownerId}`);
      }
      if (groupId !== undefined) {
        changes.push(`group: ${groupId}`);
      }
      ...
      vscode.window.showErrorMessage(
        `Failed to change ownership: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private async getFiles(
    ...
    try {
      ...
      return sortedEntries.map((entry) => {
        ...
        return new BerthTreeItemImpl(entry.name, collapsibleState, {
          type: "file",
          server,
          stack,
          fileEntry: entry,
