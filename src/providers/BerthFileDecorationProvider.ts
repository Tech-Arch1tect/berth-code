...
export class BerthFileDecorationProvider implements vscode.FileDecorationProvider {
    ...
    private async getRichTooltip(serverId: number, stackName: string, filePath: string): Promise<string> {
        try {
            ...
            if (entry.ownerId !== undefined || entry.groupId !== undefined || entry.owner || entry.group) {
                let ownerInfo = '';
                let groupInfo = '';
